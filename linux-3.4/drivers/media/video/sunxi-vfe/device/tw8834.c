...
MODULE_AUTHOR("A_T");
MODULE_DESCRIPTION("A low-level driver for tw8834 i2c-less");
MODULE_LICENSE("GPL");

//for internel driver debug
#define DEV_DBG_EN      1 
#if(DEV_DBG_EN == 1)    
#define vfe_dev_dbg(x,arg...) printk("[TW8834]"x,##arg)
#else
#define vfe_dev_dbg(x,arg...) 
#endif
#define vfe_dev_err(x,arg...) printk("[TW8834]"x,##arg)
#define vfe_dev_print(x,arg...) printk("[TW8834]"x,##arg)
...
*/
 ...
 //ret = 0;
  ...
  {
    //video
    vfe_dev_print("%s video mode set\n",__func__);
   
  } else {
    //capture image
    vfe_dev_print("%s image mode set\n",__func__);
    ...
    struct v4l2_queryctrl *qc)
{
  ...
  switch (qc->id) {
	case V4L2_CID_GAIN:
		return 0;//v4l2_ctrl_query_fill(qc, 1*16, 64*16-1, 1, 1*16);
	case V4L2_CID_EXPOSURE:
		return 0;//v4l2_ctrl_query_fill(qc, 0, 65535*16, 1, 0);
	case V4L2_CID_FRAME_RATE:
		return 30;//v4l2_ctrl_query_fill(qc, 15, 120, 1, 120);
	case V4L2_CID_VFLIP:
	case V4L2_CID_HFLIP:
